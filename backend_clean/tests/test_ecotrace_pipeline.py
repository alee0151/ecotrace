...
from backend_clean.app.pipelines.ecotrace_pipeline import (
    ArticleMetadata,
...
from backend_clean.app.services.analysis_runner import (
...
class PipelineTests(unittest.TestCase):
    ...
    def test_australia_filter_accepts_australian_source_context(self):
        ...
        with patch("backend_clean.app.services.analysis_runner.create_llm_extractor_from_env", return_value=ContextOnlyExtractor()):
    ...
    def test_australia_filter_rejects_countrywide_location_for_overseas_event(self):
        ...
        with patch("backend_clean.app.services.analysis_runner.create_llm_extractor_from_env", return_value=OverseasExtractor()):
            with patch.dict("os.environ", {"NVIDIA_API_KEY": "test-key"}):
                record = extract_one_record(
                    "BHP",
                    article,
    ...
    def test_mediastack_maps_news_response(self):
        ...
        with patch.dict("os.environ", {"MEDIASTACK_API_KEY": "test-key"}):
            with patch("backend_clean.app.services.analysis_runner.get_json", return_value=payload) as get_json:
