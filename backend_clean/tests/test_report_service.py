...
from backend_clean.app.services.report_service import (
...
class ReportServiceTests(unittest.TestCase):
    ...
    def test_email_without_smtp_writes_outbox_file(self):
        with tempfile.TemporaryDirectory() as tmp:
            with patch("backend_clean.app.services.report_service.REPORT_OUTBOX_DIR", new=Path(tmp)):
    ...
    def test_smtp_delivery_uses_configured_server(self):
        ...
        with patch.dict(os.environ, env, clear=False):
            with patch("backend_clean.app.services.report_service.smtplib.SMTP", new=FakeSMTP):
                result = deliver_report_email(
                    "analyst@example.com",
                    "Seeco verification",
                    "<html><body>Verify</body></html>",
