...
from ...db.iucn_cache import get_iucn_cache_db_status, load_iucn_cache_from_db
...
# ── CONFIG ─────────────────────────────────────────────────────────────────────
IUCN_TOKEN   = os.getenv("IUCN_TOKEN") or os.getenv("IUCN_API_TOKEN") or os.getenv("IUCN_API_KEY") or ""
...
DEFAULT_IUCN_CACHE_FILE = Path(__file__).resolve().parents[3] / "cache" / "iucn_au_cache.json"
...
class ThreatenedOccurrence:
    ...
    data_resource: Optional[str] = None
...
class LayerAResult:
    ...
    threatened_occurrences: list[ThreatenedOccurrence] = field(default_factory=list)
...
# IUCN Australia cache loaded from Postgres once per backend process.
# The IUCN API is used only by explicit migration/import scripts, not Layer A.
...
def ensure_iucn_cache_loaded() -> int:
    """
    Load the IUCN Australia cache from Postgres once per backend process.
    ...
    with _iucn_cache_lock:
        if not _iucn_au_cache:
            ...
            try:
                db_cache = load_iucn_cache_from_db(max_age_hours=0)
                if db_cache is None:
                    raise RuntimeError(
                        "IUCN database cache is empty or unavailable. "
                        "Run backend_clean/scripts/migrate_iucn_cache_to_db.py before Layer A analysis."
                    )
                _iucn_au_cache = db_cache
                _iucn_cache_status.update({
                    "state": "ready",
                    "count": len(_iucn_au_cache),
...
def reset_iucn_memory_cache() -> None:
    global _iucn_au_cache
    with _iucn_cache_lock:
        _iucn_au_cache = {}
        _iucn_cache_status.update({
...
def build_iucn_cache() -> dict:
    ...
    """
    if not IUCN_TOKEN:
        raise RuntimeError("IUCN_TOKEN/IUCN_API_TOKEN/IUCN_API_KEY is required to rebuild the IUCN cache from API")
...
def _ala_doc_value(doc: dict, *keys: str) -> Optional[str]:
    for key in keys:
        value = doc.get(key)
...
def query_ala_threatened_occurrences(
    lat: float,
    lon: float,
    radius_km: float,
    threatened_species: list[SpeciesRecord],
    per_species_limit: int = 15,
    total_limit: int = 120,
) -> list[ThreatenedOccurrence]:
    """
    Fetches mapped occurrence records for threatened species only.

    The faceted Layer A query deliberately avoids record payloads. This second,
    bounded query asks ALA for concrete occurrences with coordinates so the UI
    can place points on the map without returning thousands of records.
    """
    occurrences: list[ThreatenedOccurrence] = []
    seen: set[tuple[str, float, float, Optional[str]]] = set()
    ...
    for species in threatened_species:
        ...
        params = [
            ...
            ("lon", lon),
            ("radius", radius_km),
            ("pageSize", min(per_species_limit, total_limit - len(occurrences))),
            ("qualityProfile", "ALA"),
            ("fq", "taxonRank:species"),
            ("fq", f'taxon_name:"{species.scientific_name}"'),
            (
                "fl",
                "decimalLatitude,decimalLongitude,scientificName,vernacularName,"
                "taxon_name,eventDate,dataResourceName,basisOfRecord,occurrenceID,uuid",
            ),
        ]

        try:
            response = requests.get(ALA_OCCURRENCES_URL, params=params, timeout=TIMEOUT)
            response.raise_for_status()
            data = response.json()
        except Exception as error:
            safe_print(f"  [ALA] Occurrence fetch skipped for {species.scientific_name}: {error}")
            continue

        docs = data.get("occurrences") or data.get("records") or []
        ...
        for doc in docs:
            ...
            occurrence_lat = _as_float(doc.get("decimalLatitude") or doc.get("lat"))
            occurrence_lon = _as_float(doc.get("decimalLongitude") or doc.get("lon"))
            if occurrence_lat is None or occurrence_lon is None:
                continue

            occurrence_id = _ala_doc_value(doc, "occurrenceID", "uuid", "id")
            dedupe_key = (species.scientific_name, round(occurrence_lat, 6), round(occurrence_lon, 6), occurrence_id)
            if dedupe_key in seen:
                continue
            seen.add(dedupe_key)

            occurrences.append(ThreatenedOccurrence(
                scientific_name=_ala_doc_value(doc, "scientificName", "taxon_name") or species.scientific_name,
                common_name=_ala_doc_value(doc, "vernacularName", "commonName") or species.common_name,
                iucn_category=species.iucn_category,
                lat=occurrence_lat,
                lon=occurrence_lon,
                event_date=_ala_doc_value(doc, "eventDate"),
                data_resource=_ala_doc_value(doc, "dataResourceName"),
                basis_of_record=_ala_doc_value(doc, "basisOfRecord"),
...
def run_layer_a(lat: float, lon: float, radius_km: float = 10.0,
    ...
    # ── Step 5: Fetch mapped occurrence records for threatened species ─────────
    safe_print(f"  [ALA] Fetching mapped threatened species occurrences...")
    threatened_occurrences = query_ala_threatened_occurrences(lat, lon, radius_km, threatened)
    safe_print(f"  [ALA] Threatened occurrence points: {len(threatened_occurrences):,}")

    # ── Step 6: Calculate species threat score (0–100) ────────────────────────
    ...
    result = LayerAResult(
        ...
        threatened_occurrences = threatened_occurrences,
...
if __name__ == "__main__":
    ...
    output = {
        ...
        "threatened_occurrences": [
            {
                "scientific_name": occurrence.scientific_name,
                "common_name": occurrence.common_name,
                "iucn_category": occurrence.iucn_category,
                "lat": occurrence.lat,
                "lon": occurrence.lon,
                "event_date": occurrence.event_date,
                "data_resource": occurrence.data_resource,
                "basis_of_record": occurrence.basis_of_record,
                "occurrence_id": occurrence.occurrence_id,
            }
            for occurrence in result.threatened_occurrences
        ],
        "all_species": [
            {
                "scientific_name": s.scientific_name,
